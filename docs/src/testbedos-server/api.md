...
- `/api/config`: which is for editing the `kvm-compose-config`.
  <br>This supports GET and POST, to get and update respectively.

- `/api/deployments`: which is for controlling the testbed deployments.
  <br>This supports GET and POST, to get and update the deployments respectively.

- `/api/deployments/:name`: which is for controlling specific testbed deployments.
  <br>The `:name` will be the name of the deployment.
  <br>This supports GET, DELETE and PUT, to get, delete and update the specific deployment respectively.

- `/api/deployments/:name/action`: which is for applying commands to testbed deployments.
  <br>This supports POST, to apply a specific testbed action such as generate-artefacts.

- `/api/deployments/:name/state`: which is for interacting with an existing state for the deployment.
  <br>This supports GET, to get the state json file for the deployment.
