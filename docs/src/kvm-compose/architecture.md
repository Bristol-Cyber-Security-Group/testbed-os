...
It uses the serde library to deserialise and parse the [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) file to start building a state representation in memory.
The state representation, once enumerated with details from the [kvm-compose.yaml](./kvm-compose/kvm-compose-yaml/index.md#kvm-compose-yaml) and [`kvm-compose-config.json`](../testbed-config/index.md#testbed-config), will be also serialised with serde into the state JSON file to be used with [orchestration](../orchestration/index.md#orchestration).
Along with the state JSON, the artefacts are also generated.
Note: this CLI tool uses the APIs on the [testbed server](../testbedos-server/index.md#testbedos-server) to execute the testbed processes.
...
The orchestration tool "doesn't care" what is inside these, as long as the generated [state JSON](#state-json) file is valid.

`kvm-compose` needs access to the libvirt daemon to access information about existing libvirt networks.

See the [networking](../networking/index.md#networking) section for more information on the architecture for generating the distributed bridges and tunnels configuration.

## Load Balancing

Given an arbitrary network topology and machine definitions in the [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) file, these will be distributed over the testbed hosts listed in the [`kvm-compose-config.json`](../testbed-config/index.md#testbed-config) file.
...
Starting on the first host in the [`kvm-compose-config.json`](../testbed-config/index.md#testbed-config) file, each bridge is allocated until all bridges allocated.
...
The artefacts generated are placed in the `artefacts` folder inside the project folder, that contains the [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) file.
...
The state JSON file is a direct serialisation of the desired state from the [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) file in memory.
...
See the [testbed server](../testbedos-server/index.md#testbedos-server) section for details on the architecture of the log streaming.
