...
It uses a yaml file [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) as the source of configuration that describes a test case to be deployed on the testbed.
...
The artefacts folder will contain the various scripts and config files to be used to express the configuration in the state JSON file and therefore the original [kvm-compose.yaml](./kvm-compose-yaml/index.md#kvm-compose-yaml) file.

This tool also offers the ability to create the [kvm-compose-config.json](../testbed-config/index.md#testbed-config), which needs to be enumerated with the testbed host information.
