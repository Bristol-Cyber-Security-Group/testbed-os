...
For more advanced usage of the testbed, for example using it in a cluster mode, you will need to further configure the `host.json` so please see [`kvm-compose-config.json`](../testbed-config/index.md#testbed-config) if that is the case.
...
- **[Rust](https://rustup.rs/)**: Default configuration, when asked, is fine.
- **[Poetry](https://python-poetry.org/docs/#installation)**: Make sure you have python `^3.10`, consider using pyenv to manage python installs (see [poetry documentation](https://python-poetry.org/docs/managing-environments/)). Also make sure you have pip3 installed for this python version, for ubuntu install `python3-pip`.
- **[PyEnv](https://github.com/pyenv/pyenv)**: Python version manager, asks you to manually add it to your shell profile once installed. 
...
Clone the testbed git repo into your desired location then navigate to the root directory. Execute:
...
See [`kvm-compose-config.json`](../testbed-config/index.md#testbed-config) documentation for more information.
...
See [orchestration](../orchestration/index:orchestration) for more information on how to deploy a test case.
...
Please see the testbed server [Cluster Management](../testbedos-server/architecture.md#cluster-management) for more information.
...
You should tear down any test cases before uninstalling the testbed, see [orchestration](../orchestration/index.md#orchestration) for more information on how to tear down a test case.
