...
pub async fn configure_testbed_host(
    ...
    run_subprocess_command_allow_fail(
        "ip",
        vec!["addr", "add", ip.as_str(), "dev", host_config.testbed_nic.as_str()],
        false,
        None,
    ).await?;
    // make sure it is up
    run_subprocess_command_allow_fail(
        "ip",
        vec!["link", "set", host_config.testbed_nic.as_str(), "up"],
        false,
        None,
    ).await?;
    ...
    let main_interface = if host_config.main_interface.is_some() {
        ...
        host_config.main_interface = Some(get_default_interface().await.context("getting default interface")?);
        host_config.main_interface.as_ref().unwrap()
    };

    // configure any OVN related settings and make sure ovn and ovs are up, before other services
    configure_host_ovn(&host_config.ovn, &main_interface, is_main, &host_config)
        .await.context("configure host ovn")?;

    // make sure libvirt, docker are up
    // ensure_services_up().await.context("ensure system services are up")?;
    ...
    match mode {
        ...
        ServerModeCmd::Main => {
            manage_cluster(&ClusterOperation::Init, db_config)
                .await.context("init cluster")?;
        }
        ServerModeCmd::CreateConfig => {}
    }

    // we may have edited the configs or filled in any default values, so write any changes
    db_config.write().await.set_host_config(host_config).await.context("update host config after startup")?;
...
pub async fn manage_cluster(
    ...
    let host_config = db_config
        ...
        .get_host_config()
        .await
        .context("reading host configuration")?;
    ...
    let mut cluster_config = TestbedClusterConfig {
        ...
        ovs_db_socket: "".to_string(),
        ovn_nb_db_docket: "".to_string(),
        ovn_sb_db_socket: "".to_string(),
        docker_socket: "".to_string(),
    };
    TestbedClusterConfig::insert_default_values(&mut cluster_config);
    // save new config to disk
    db_config.write().await.set_cluster_config(cluster_config.clone())
        .await.context("writing host configuration")?;
    ...
    match cluster_operation {
        ClusterOperation::Init => {
            ...
            let mut cluster_config = db_config.read().await.get_cluster_config()
                .await.context("getting cluster configuration for init")?;
            tracing::debug!("getting host config");
            let host_config = db_config.read().await.get_host_config()
                .await.context("getting host configuration for init")?;
            ...
            db_config.write().await.set_cluster_config(cluster_config)
                .await.context("setting updated cluster configuration for init")?;
        }
