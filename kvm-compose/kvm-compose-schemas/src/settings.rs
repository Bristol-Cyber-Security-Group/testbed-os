...
pub struct SshConfig {
    ...
    /// master testbed, not locally - so the master knows how to connect to this host
    ...
    /// must be set to Some(true) if this is the master, otherwise will be regarded as a client host
    // TODO - with cluster management, we can default this value based on the mode.json
    pub is_master_host: Option<bool>,
    /// this is the OVN configuration for this host
    #[serde(default)]
    pub ovn: OvnConfig, // default settings are for master
}
...
/// that will work for master mode. When in client mode, the client_ovn_remote will be filled by
/// the master testbed as a response from joining the cluster. Therefore you only need to fill the
...
pub struct OvnConfig {
    ...
    #[serde(default = "default_master_ovn_remote")]
    pub master_ovn_remote: String,
    /// this is the ip of the remote OVN southbound database, to be used when this host is a client
    /// in a cluster
    #[serde(default)]
    pub client_ovn_remote: Option<String>, // for connecting to a master testbed ovn cluster
...
impl Default for OvnConfig {
    fn default() -> Self {
        Self {
            ...
            master_ovn_remote: default_master_ovn_remote(),
...
fn default_master_ovn_remote() -> String {"unix:/usr/local/var/run/ovn/ovnsb_db.sock".to_string()}
